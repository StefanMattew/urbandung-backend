...
app.patch('/api/kuliners/:id', async (req, res) => {
    try {
        ...
        const { crowdStatus } = req.body;
        if (!id) {
            return res.status(400).json({ error: "ID Kuliner tidak ditemukan" });
        }
        const updatedKuliner = await prisma.kuliner.update({
            where: { id: parseInt(id) },
            data: { crowdStatus: crowdStatus }
        });
        res.status(200).json(updatedKuliner);
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Terjadi kesalahan pada server saat update status" });
    }
...
app.get('/api/kuliners/:id/reviews', async (req, res) => {
    try {
        const reviews = await prisma.kulinerReview.findMany({
            where: { kulinerId: parseInt(req.params.id) },
            include: { user: { select: { name: true } } },
            orderBy: { createdAt: 'desc' }
        });
        res.json(reviews);
